...
# The 12,500.00 transfer crosses the 10,000 review threshold, so it is HELD:
# the admin console shows it awaiting operator approval (Approve / Decline).
...
# Large transfer: crosses the 10,000 review threshold and stays HELD until an
# operator approves it (same key on rerun, so it is recorded exactly once).
...
Write-Output "Seeded: alice@bank.local / bob@bank.local (password secret123). Alice sent Bob `$250; her `$12,500 wire is HELD awaiting operator approval (Approve it in Operations to settle)."
