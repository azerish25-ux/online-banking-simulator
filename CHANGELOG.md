...
- **A transfer to an IBAN that isn't an account here explains itself**: same 404, but the detail says the recipient must have an account in the
...
- **Credit is capped**: a customer could previously open unlimited LOAN
  accounts and mint unbounded $1,000 credit. One open loan per user now: service-level check plus a PostgreSQL partial unique index (V15, Java
...
- **TOTP is complete end to end**: `totpEnabled` on `/me`, a Security page to
  set up/enable/disable the authenticator, and a `/login/mfa` challenge screen: before this, enabling 2FA made web login impossible.
